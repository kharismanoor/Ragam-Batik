...
<nav class="fixed top-0 z-50 w-full bg-red-900 border-b border-gray-200">
  ...
  <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-gray-200 border-r border-gray-200 sm:translate-x-0" aria-label="Sidebar">
     <div class="h-full px-3 pb-4 overflow-y-auto bg-gray-200">
        <ul class="space-y-2 font-medium">
           ...
           <li>
            <a href="/order" class="flex items-center p-2   text-white bg-yellow-600 group  rounded-lg hover:text-white hover:bg-yellow-600 group">
  ...
  <div class="p-10 sm:ml-64">
    ...
    <table class="w-full text-sm text-left rtl:text-right text-blue-100 dark:text-black">
        <thead class="text-xs text-white uppercase bg-gray-100 dark:text-black">
            <tr>
        ...
        <tbody>
          @foreach ($orders as $order)
            <tr class="bg-white border-b border-red-950">
                <td class="px-6 py-4">
                  {{ $order->id }}
                </td>
                <td class="px-6 py-4">
                ...
                <td >
                  <form  action="{{ route('order.destroy', $order->id) }}" method="POST">
                    <a href="{{ route('order.edit', $order->id) }}" class="font-medium text-white bg-red-900 hover:bg-red-300 hover:text-red-600 rounded-lg text-sm px-5 py-2 text-center mb-2 items-center w-40 m-2 hover:underline">Detail</a>
                  </form>
                </td>
            </tr>
            @endforeach
